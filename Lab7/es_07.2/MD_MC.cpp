...
int main()
{
  ...
  for (int i = 0; i < 3; i++)
  {
    state = states[i];
    cout<<"-------------------------------------------"<<endl;
    cout<<"Performing MD simulations via a Lennard-Jones model of Argon in a "<<state<<" state"<<endl;
    ...
    for (int iblk = 1; iblk <= nblk; iblk++) // Simulation
    {
      ...
      {
        printProgressBar((double)((iblk - 1) * nstep + istep) / (nblk * nstep));
        Move();
        Measure();
      }
...
void Measure() // Properties measurement
{
  ...
  ofstream Inst_press , Inst_ene ;

  Inst_press.open(dir_out+"instant_pressure.dat",ios::app) ;
  Inst_ene.open(dir_out+"instant_energy.dat",ios::app) ;
  ...
  Inst_press << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl ;
  Inst_ene << walker[iv]/(double)npart + vtail << endl;
