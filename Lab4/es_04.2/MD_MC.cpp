...
int main()
{
  //string states[3] = {"TH_gas", "TH_liquid", "TH_solid"}; //for thermalization simulation
  string states[1] = {"gas"};//, "liquid","solid"}; //for final simulation
  ...
  for (int i = 0; i < 3; i++)
  {
    state = states[i];
    cout<<"-------------------------------------------"<<endl;
    cout<<"Performing MD simulations via a Lennard-Jones model if Argon in a "<<state<<" state"<<endl;
...
void Input()
{
  ...
  ReadInput.open(state+"_simulation/input." + state);
  ...
  if (restart)
  {
    ReadConf.open(state + "_simulation/" + "config.out");
    ReadVelocity.open(state + "_simulation/" + "velocity.out");
...
void Averages(int iblk) // Print results for current block
{
  ...
  cout << "Acceptance rate " << ((double)accepted / attempted)*100<<" %" << endl
       << endl;}

  Epot.open(state + "_simulation/" + "output_epot.dat", ios::app);
  Ekin.open(state + "_simulation/" + "output_ekin.dat", ios::app);
  Temp.open(state + "_simulation/" + "output_temp.dat", ios::app);
  Etot.open(state + "_simulation/" + "output_etot.dat", ios::app);
  Pres.open(state + "_simulation/" + "output_pres.dat", ios::app);
...
void ConfFinal(void)
{
  ...
  cout << "Print final configuration to file config.out" << endl
       << endl;
  WriteConf.open(state + "_simulation/" + "config.out");
  WriteVelocity.open(state + "_simulation/" + "velocity.out");
  for (int i = 0; i < npart; ++i)
  {
